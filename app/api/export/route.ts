    ...
    case "clients": {
      const { rows } = await sql`
        SELECT c.client_id, c.name, c.family_size, c.point_budget,
               ...
               c.service_number, c.notes, c.delivery_approved,
      ...
      `;
      return {
        headers: ["Client ID", "Name", "Family Size", "Credits", "Date of Birth", "Gender", "Contact", "Email", "Address", "Service Number", "Allergies / Notes", "Delivery Approved", "Status", "Archive Reason", "Created", "Members On File"],
        rows: rows.map((r) => [r.client_id, r.name, r.family_size, r.point_budget, r.date_of_birth ? String(r.date_of_birth) : "", r.gender, r.contact, r.email, r.address, r.service_number, r.notes, r.delivery_approved ? "yes" : "no", r.status, r.archive_reason, new Date(r.created_at).toISOString().slice(0, 10), r.members]),
      };
    }
    case "family_members": {
      const { rows } = await sql`
